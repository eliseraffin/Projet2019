					...
					for (int k=0; k<3; k++) {
						if (2*j+j+k==i && this.mdl.objets_inventaire[j][k].lib != null) {
							...
							if (this.mdl.objets_inventaire[j][k].qte >1) {
								this.mdl.nouvelObjet(nom,1);
							} if (this.mdl.objets_inventaire[j][k].qte==0) {
								this.mdl.nouvelObjet(null,0);
							} if (this.mdl.objets_inventaire[j][k].qte==1) {
								this.mdl.nouvelObjet(nom,1);
								this.mdl.objets_inventaire[j][k] = new Objet("vide",0);	
							} if (this.mdl.a_stocker.lib!=null && this.mdl.a_stocker.lib ==nom) {
								this.mdl.objets_inventaire[j][k] = new Objet(nom,this.mdl.a_stocker.qte+this.mdl.objets_inventaire[j][k].qte);	
							}
							...
							this.mdl.ajouterRecette(this.mdl.a_stocker.lib, this.mdl.ind_recipe,this.mdl.a_stocker.qte);
							this.mdl.a_stocker = new Objet(null,0);
						...
						if (2*i+i+k==indice) {
							if (this.mdl.crafting[i][k].lib == null) {
								if (!(this.mdl.est_present_invt(this.mdl.objt_select.lib))) {
									this.mdl.ajouterObjet(this.mdl.objt_select.lib, indice,1);
									this.mdl.objt_select = new Objet(null,0);
								...
								for (int m = 0; m<5; m++) {
									for (int n = 0; n<3; n++) {
										...
										if (this.mdl.objets_inventaire[m][n].lib==this.mdl.objt_select.lib && this.mdl.objets_inventaire[m][n].qte==1) {
											...
											this.mdl.objets_inventaire[m][n] = new Objet("vide",0);	
										} 	
									}
								} 
							} else {
								if (this.mdl.crafting[i][k].lib != null) {
									...
									this.mdl.ajouterObjet(null, indice,0);
									if (this.mdl.est_present_invt(o)) {
										...
										this.mdl.remettre_objet_inventaire(o, this.mdl.indice_remettre_objet_inventaire());
