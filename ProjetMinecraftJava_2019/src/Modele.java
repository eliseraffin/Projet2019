...
public class Modele extends Observable{
	...
	public Modele() {
		...
		this.affichage = "";
		Objet baton = new Objet("baton",6);
		Objet bois = new Objet("bois",6);
		Objet canne_sucre = new Objet("canne_sucre",6);
		Objet fil = new Objet("fil",6);
		Objet lingot_fer = new Objet("lingot_fer",6);
		Objet pierre = new Objet("pierre",6);
		Objet plume = new Objet("plume",6);
		Objet redstone = new Objet("redstone",6);
		Objet silex = new Objet("silex",6);
		Objet arc = new Objet("arc",1);
		Objet boussole = new Objet("boussole",1);
		Objet planche = new Objet("planche",9);
		Objet papier = new Objet("papier",9);
		Objet nul = new Objet(null,0);

		Objet canne_a_peche = new Objet("canne_peche",1);
		Objet hache_objet = new Objet("hache",1);
		Objet pioche_objet = new Objet("pioche",1);
		Objet four_objet = new Objet("four",1);
		Objet distributeur_objet = new Objet("distrib",1);
		Objet cisaille_objet = new Objet("cisaille",1);
		Objet carte_objet = new Objet("carte",1);
		Objet bateau_objet = new Objet("bateau",1);
		Objet bol_objet = new Objet("bol", 1);
		Objet barriere_objet = new Objet("barriere", 1);
		Objet briquet_objet = new Objet("briquet",1);
		Objet fleche_objet = new Objet("fleche",1);
	...
	public void ajouterObjet(String obj, int ind,int qte) {	
		for (int i = 0; i<3; i++) {
			for (int j = 0; j<3; j++) {
				if (2*i+j+i == ind) {
					this.crafting[i][j] = new Objet(obj,1);
					this.ind = ind;
					this.setChanged();
	...
	public void ajouterRecette(String obj, int ind,int qte) {
		for (int i = 0; i<5; i++) {
			for (int j = 0; j<3; j++) {
				if (2*i+i+j==ind && (this.objets_inventaire[i][j].lib==null || this.objets_inventaire[i][j].lib=="vide")) {
					this.objets_inventaire[i][j] = new Objet(obj,qte);
					this.ind_recipe = ind;
					this.a_stocker = new Objet(null,0);
					this.ind_recipe = 15;
					this.setChanged();
	...
	public void raz_recette() {
		this.r=new Recette(null,new Objet[3][3],new Objet(null,1), new HashMap<String,Integer>(), false);
		this.setChanged();
		this.notifyObservers(this.r);
	}


	public void nouvelObjet(String lib, int qte) {
		this.objt_select = new Objet(lib,qte);
		this.ind = 10;
		this.setChanged();
	...
	public void raz_craft() {
		for (int i = 0; i<3; i++) {
			for (int j = 0; j<3; j++) {
				this.crafting[i][j] = new Objet(null,0);
				this.ind = 10;
			}
		}
	...
	public void remettre_objet_inventaire (String ob,int ind) {
		for (int i = 0; i<5; i++) {
			for (int j = 0; j<3; j++) {
				if (2*i+i+j == ind) {
					this.objets_inventaire[i][j] = new Objet(ob,1);
				}
			}
		}
